#!/usr/bin/env python3
"""Test selected technical filters on top of current F strategy over recent 100 trading days."""
...
OUT_PATH = PROJECT_ROOT / "backtest" / "strategy_f_filter_effectiveness_recent_100d.json"
...
def make_filter_backtest(module, filter_name: str | None):
    class FilterBacktest(module.Backtest):
        def _passes_extra_filter(self, row) -> bool:
            ...
            if filter_name == "rs60_top20pct_and_industry_20_60_gt_market":
                rs_ok = pd.notna(row.get("rs60_pct")) and float(row.get("rs60_pct")) >= 0.8
                ind_ok = (
                    pd.notna(row.get("ind_ret20")) and pd.notna(row.get("ind_ret60"))
                    and pd.notna(row.get("idx_ret20")) and pd.notna(row.get("idx_ret60"))
                    and float(row.get("ind_ret20")) > float(row.get("idx_ret20"))
...
def main():
    ...
    filters = [
        ...
        "rs60_top20pct_and_industry_20_60_gt_market",
    ]
